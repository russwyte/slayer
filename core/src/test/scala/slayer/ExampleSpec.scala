...
/** Showcase spec — demonstrates what slayer is for: writing focused tests against ZIO services without writing a
  * fake implementation by hand. Each test highlights a capability you'd otherwise miss with a hand-rolled stub.
  *
...
object ExampleSpec extends ZIOSpecDefault:
  ...
  /** A multi-parameter-list (curried) collaborator — the kind of API where you pass context first and data second.
    * stubby couldn't handle these for handler functions; slayer reads the *handler*'s function shape, so either
    * tupled or curried handlers are accepted.
    */
  trait AuditLog:
    ...
    test("handlers can capture calls — verify Notifier was invoked with the expected message") {
      val program =
        for
          ...
          _     <- stub[Notifier](_.notify(slayer.any[OrderId], slayer.any[String])) {
                     (id: OrderId, msg: String) => calls.update((id, msg) :: _)
                   }
          svc   <- ZIO.service[OrderService]
